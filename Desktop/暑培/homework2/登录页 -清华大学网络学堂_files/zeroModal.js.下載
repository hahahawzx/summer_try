...
(function(obj) {
    ...
    var defaultOpt = {
        ...
        okTitle: wlxtlang_queding,//'确定', //“ok”按钮的显示值，默认为“确定”
        okFn: false, //点击“ok”按钮触发的事件
        cancel: false, //是否启用“cancel”按钮，默认false
        cancelTitle: wlxtlang_quxiao,//'取消', //“cancel”按钮的显示值，默认为“取消”
        cancelFn: true, //点击“cancel”按钮触发的事件
        buttonTopLine: true,
        buttons: [], //自定义的按钮，使用了自定义按钮ok与cancel按钮将不会生效；格式：[{ className: 'btn btn-primary', name: '开始导出', fn:function(opt){} }]
    ...
    zeroModal.zuoye = function(obj) {//debugger;
        var params;
        params = _initParams();
        _tmp_last_zindex++;
        var _width = 300;
        ...
        var modalclass = "zeromodal-container";
        //// 构建容器
        var _text = obj.title+"“...”批阅提交成功";

        var _closebtn = '<p class="closebtn" onclick="closeModal(this)"><i class="webicon-g_close"></i></p>';
        var _container = $('<div class="zuoyemodal" style="width:'+_width+'px;height:'+_height+'px"><p class="text">'+_text+'</p>'+_closebtn+'</div>');
     ...
     * @return {[type]}       [description]
     */
    var lskcdrprogressunique;
    zeroModal.lskcdrprogressbar = function(percent, time ,jdmc) {
        ...
        var modalclass = "zeromodal-container progess_con";
        //// 构建容器
       
        var wlxtlang_zhengzaidaorushaodeng1 = wlxtlang_zhengzaidaorushaodeng.replace("{0}",jdmc);
        var _text = wlxtlang_zhengzaidaorushaodeng1;//"正在导入"+jdmc+"请稍等.....";
        if ($("#text").size() == 0) {
            var _progressbar = '<div class="progress"><div class="bar" style="width:0"><span id="text">0%</span></div></div><p>' + _text + '</p><span class="time restTime">' + time + '</span>';
            var _container = $('<div zero-unique-container="' + params.unique + '" class="' + modalclass + '" style="z-index:' + _tmp_last_zindex + ';width:' + _width + 'px;height:' + _height + 'px;left:' + _left + 'px;top:' + _top + 'px">' + _progressbar + '</div>');
            $('body').append(_container);
            lskcdrprogressunique = params.unique;
            $('body').append('<div class="greymask" id="greymask" style="position:absolute;width:100%;height:100%;z-index:99;"></div>');
        } else {
            //debugger;
            params.unique;
        ...
        if (percent == 100) {
            ...
            $('[zero-unique-container="' + lskcdrprogressunique + '"]').remove();
            turnon = true;
            $("#greymask").remove();
        }
    }
     ...
     */
    var progressunique;
    zeroModal.progressbar = function(percent, time) {
        var params;
        params = _initParams();
        _tmp_last_zindex++;
        var _width = 300;
        ...
        var modalclass = "zeromodal-container progess_con";
        //// 构建容器
        var _text = wlxtlang_wenjianzhengzaishangchuan+".....";
        if ($("#text").size() == 0) {
            var _progressbar = '<div class="progress"><div class="bar" style="width:0"><span id="text">0%</span></div></div><p>' + _text + '</p><span class="time restTime">' + time + '</span>';
            var _container = $('<div zero-unique-container="' + params.unique + '" class="' + modalclass + '" style="z-index:' + _tmp_last_zindex + ';width:' + _width + 'px;height:' + _height + 'px;left:' + _left + 'px;top:' + _top + 'px">' + _progressbar + '</div>');
            $('body').append(_container);
            progressunique = params.unique;
            $('body').append('<div class="greymask" id="greymask" style="position:absolute;width:100%;height:100%;z-index:99;"></div>');
        } else {
            //debugger;
            params.unique;
        ...
        if (percent == 100) {
            ...
            $('[zero-unique-container="' + progressunique + '"]').remove();
            turnon = true;
            $("#greymask").remove();
        }
    }
    ...
    zeroModal.alert = function(content) {
        var _opt = {
            ...
            iconText: '<i class="icon iconfont icon-Warning"></i>'
        };
    ...
    zeroModal.error = function(content) {
        var params = {
            iconDisplay: '<div class="show-zero2 zeromodal-error"><i class="webicon-t_close"></i></div>'
        };
    ...
    zeroModal.notice = function(content) {
        var params = {
            iconDisplay: '<div class="show-zero2 zeromodal-error"><i class="webicon-warning"></i></div>'
        };
    ...
    zeroModal.success = function(content) {
        var params = {
            iconDisplay: '<div class="show-zero2 zeromodal-success"><i class="webicon-t_check"></i></div>'
        };
        if (typeof content === 'object') {
            $.extend(params, content);
        } else {
     ...
     * @return {[type]}         [description]
     */
    zeroModal.confirm = function(content, okFn, width) {
        var _opt = {
            iconClass: 'show-zero2 zeromodal-icon-question',
            iconText: '<i class="webicon-t_question"></i>',
        };
    ...
    function _buildModal(opt) {
        ...
        var _top = $(window).scrollTop() + Math.ceil((screenHeight-150 - _height) /3);
        var modalclass = "zeromodal-container";
        //// 构建容器
        if (_css != ' ') {
            modalclass = modalclass + ' ' + _css;
        }
        if (opt.nohead == true) {
            modalclass = modalclass + ' alert ';
        }
        var _container = $('<div zero-unique-container="' + opt.unique + '" class="' + modalclass + '" style="z-index:' + _tmp_last_zindex + ';width:' + _width + 'px;height:' + _height + 'px;left:' + _left + 'px;top:' + (opt.transition ? _top - 50 : _top) + 'px"></div>');
        $('body').append(_container);
        ...
        if (opt.drag) {
            ...
            new Drag($('[zero-unique-container="' + opt.unique + '"]')[0], {
                handle: _handle,
                limit: false
            });
        }
        ...
        if (opt.nohead != true) {
            ...
            _headerHtml += '        <div title="关闭" zero-close-unique="' + opt.unique + '" class="zeromodal-close"><i class="webicon-g_close"></i></div>';
            if (opt.max) {
    ...
    function _buildFooter(opt, _container) {
        ...
        if (opt.ok || opt.cancel || (opt.buttons !== undefined && opt.buttons.length > 0)) {
            var _footer = '<div class="zeromodal-footer '+footercss+'">';
            ...
            if (opt.buttons !== undefined && opt.buttons.length > 0) {
                ...
                // 显示默认提供的按钮
                if (opt.ok) {
                    var _ok = $('<button zeromodal-btn-ok="' + opt.unique + '" class="btn btn-primary">' + opt.okTitle + '</button>');
                ...
                if (opt.cancel) {
                    var _cancel = $('<button zeromodal-btn-cancel="' + opt.unique + '" class="btn btn-cancel">' + opt.cancelTitle + '</button>');
    ...
    function _buildAlertInfo(opt) {
        ...
        if (typeof opt === 'undefined' || typeof opt.cancelTitle === 'undefined') {
            opt.cancelTitle = wlxtlang_quxiao;//'取消';
        }

        var params = _initParams(opt);
        params.width = opt.width || '500px';
        ...
        if (typeof params.iconDisplay !== 'undefined') {
            ...
            icon = $('<div class=" ' + params.iconClass + '">' + params.iconText + '</div>');
        }
    ...
    function _resizeBodyHeight(opt) {
        ...
        var height = $('[zero-unique-container="' + opt.unique + '"]').height() - headerHeight - buttonHeight;
